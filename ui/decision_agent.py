...
def render_decision_agent(decision_data: dict):
    ...
    if not decision_data:
        st.info("Waiting for lead profile and property matches to run the AI Decision Engine...")
        return
    ...
    with col1:
        ...
        if action == "ESCALATE_TO_BROKER":
            st.success(f"**NEXT ACTION:**\n\n### {action} 🔥")
        elif action == "LOW_PRIORITY":
            st.error(f"**NEXT ACTION:**\n\n### {action} 🧊")
        elif action == "SEND_PROPERTY_SHORTLIST":
            st.info(f"**NEXT ACTION:**\n\n### {action} 📄")
        else:
            st.warning(f"**NEXT ACTION:**\n\n### {action} 💬")
